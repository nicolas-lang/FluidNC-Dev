...
namespace Spindles {
    ...
    protected:
        ...
        void tool_change_manual(uint8_t new_tool);
        void tool_preselect(uint8_t new_tool);

        void go_above_tool(uint8_t tool_num);
        bool take_tool(uint8_t tool_num);
        bool return_tool(uint8_t tool_num);

        bool set_ATC_state(bool open);
        bool atc_toolsetter_probe();
        void goto_top_of_z();
        void atc_ETS_dustoff();
        ...
        const float TOOL_GRAB_TIME = 0.5;  // seconds. How long it takes to grab a tool
