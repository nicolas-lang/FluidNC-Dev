...
namespace Spindles {
    ...
    bool KressATC::tool_change(uint8_t new_tool, bool pre_select) {
        ...
        if (gc_state.modal.spindle != SpindleState::Disable) {
            spindle_was_on = true;
            gc_exec_linef(true, "M5");  // this should add a delay if there is one
        ...
        if (!return_tool(current_tool)) {  // does nothing if we have no tool
            gc_exec_linef(true, "G53 G0 X%0.3f Y%0.3f", tool[new_tool].mpos[X_AXIS], tool[new_tool].mpos[Y_AXIS]);
        }

        current_tool = 0;  // now we have no tool

        if (new_tool == 0) {  // if changing to tool 0...we are done.
            gc_exec_linef(true, "G53 G0 Y%0.3f", tool[0].mpos[Y_AXIS] - RACK_SAFE_DIST_Y);
        ...
        gc_exec_linef(true, "G53 G0 Z%0.3f", tool[new_tool].mpos[Z_AXIS]);  // drop down to tool
    ...
    void KressATC::go_above_tool(uint8_t tool_num) {
        if (current_tool != 0)
            ...
            gc_exec_linef(false, "G53 G0 Z%0.3f", _empty_safe_z);

        if (current_tool != 0) {
            // move in front of tool
            gc_exec_linef(false, "G53 G0 X%0.3f Y%0.3f", tool[tool_num].mpos[X_AXIS], tool[tool_num].mpos[Y_AXIS] - RACK_SAFE_DIST_Y);
        }
        // Move over tool
        gc_exec_linef(true, "G53 G0 X%0.3f Y%0.3f", tool[tool_num].mpos[X_AXIS], tool[tool_num].mpos[Y_AXIS]);
    }
    ...
    bool KressATC::atc_toolsetter() {
        ...
        if (current_tool == 1) {
            ...
            gc_exec_linef(true, "G53 G0 X%0.3f Y%0.3f", tool[ETS_INDEX].mpos[X_AXIS], tool[ETS_INDEX].mpos[Y_AXIS]);  // Move over tool
        } else {
            gc_exec_linef(false, "G91");
            // Arc out of current tool
            gc_exec_linef(false, "G2 X-%0.3f Y-%0.3f I-%0.3f F4000", RACK_SAFE_DIST_Y, RACK_SAFE_DIST_Y, RACK_SAFE_DIST_Y);
            ...
            // arc in
            gc_exec_linef(false, "G2 X-%0.3f Y%0.3f J%0.3f F4000", RACK_SAFE_DIST_Y, RACK_SAFE_DIST_Y, RACK_SAFE_DIST_Y);
            gc_exec_linef(false, "G90");
            // Move over tool
        ...
        gc_exec_linef(true, "G38.2 F%0.3f Z%0.3f", PROBE_FEEDRATE, probe_to);  // probe
        ...
        goto_top_of_z();
        // move forward
        gc_exec_linef(false, "G53 G0 X%0.3f Y%0.3f", tool[ETS_INDEX].mpos[X_AXIS], tool[ETS_INDEX].mpos[Y_AXIS] - RACK_SAFE_DIST_Y);
